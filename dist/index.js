                ...
                try {
                    ...
                    if (results.headings) {
                        ...
                        if (h.issues.length > 0)
                            detailsText += `- 이슈: ${h.issues.join(", ")}\n`;
                        detailsText += "\n";
                    }
                    if (results.landmarks) {
                        const l = results.landmarks;
                        detailsText += `### Landmarks ${l.pass ? "✅" : "❌"}\n`;
                        detailsText += `- 발견: ${l.found.map((f) => `${f.role}(${f.count})`).join(", ") || "없음"}\n`;
                        if (l.issues.length > 0)
                            detailsText += `- 이슈: ${l.issues.join(", ")}\n`;
                        detailsText += "\n";
                    }
                    if (results.ariaLabels) {
                        ...
                        if (a.issues.length > 0) {
                            ...
                            if (a.issues.length > 5)
                                detailsText += `  - ... 외 ${a.issues.length - 5}개\n`;
                        }
                        detailsText += "\n";
                    }
                    if (results.tabOrder) {
